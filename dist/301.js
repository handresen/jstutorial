...
function getGameTypeString(gameSettings) {
    ...
    if (!gameSettings.DoubleIn && gameSettings.DoubleOut) s += " DOUBLE OUT";else if (gameSettings.DoubleIn && gameSettings.DoubleOut) s += " DOUBLE IN & OUT";
    return s;
}
...
function removeLastHit(turn) {
    if (turn.childNodes.length > 1) turn.removeChild(turn.lastChild);
}
...
function applyValue(playeridx, turnidx, value, isdouble) {
    ...
    if (GameSettings.DoubleIn && initialscore == GameSettings.StartValue && !isdouble) value = 0;
...
function processButton(payloadString) {
    ...
    if (payloadString == "5") {
        ...
        if (GameSettings.Players > 4) GameSettings.Players = 1;
        setup();
        return;
    }
    if (payloadString == "6") {
    ...
    if (payloadString == "8") {
        if (GameSettings.StartValue == 301) GameSettings.StartValue = 501;else GameSettings.StartValue = 301;
        resetScores();
        setup();
    }
    if (payloadString == "9") {
...
function replayMessages() {
    ...
    while (true) {
        ...
        if (hit != null) processHit(hit);else if (btn != null) processButton(btn);else return;
...
function recordMessage(name, payload) {
    if (typeof Storage !== "undefined") {
        s = "";
        if (name == "dart01/hits") s = "h_" + messageIndex++;
        if (name == "dart01/buttons") s = "b_" + messageIndex++;
...
function processMessage(name, payload, record) {
    if (record) recordMessage(name, payload);
    if (name == "dart01/hits") processHit(payload);else if (name == "dart01/buttons") processButton(payload);
}
